            ...
            try:
                ...
                if action == "create":
                    ...
                    if username == lobby["creator"]:
                        ...
                        await websocket.send_json({"action": "creator_disconnected", "message": "Lobby closed"})
                    else:
                        if username in lobby["players"]:
        ...
        if websocket in client_list:
            ...
            for creator, lobby in list(lobbies.items()):
                if lobby["lobby_id"] == lobby_id:
                    if lobby["creator"] in lobby["players"]:
                        # Check if the disconnecting client was the creator
                        creator_websocket = None
                        for client in client_list:
                            if client == websocket and lobby["creator"] in lobby["players"]:
                                creator_websocket = client
                                break
                        if creator_websocket:
                            # Creator disconnected
                            for client in client_list:
                                if client != creator_websocket:
                                    try:
                                        await client.send_json({"action": "creator_disconnected", "error": "Lobby closed by creator"})
                                    except Exception as e:
                                        print(f"Error notifying client in lobby {lobby_id}: {e}")
                            del clients[lobby_id]
                            del lobbies[creator]
                            print(f"Lobby {lobby_id} deleted due to creator {lobby['creator']} disconnect")
                        else:
                            # Non-creator player disconnected
